var nbMuseesParQuartier = [];
for (var i = 0; i < 80; i++) {
    nbMuseesParQuartier.push(0);
}
...
for (var i = 0; i < tab.length; i++) {
    for (var j = 0; j < polygons.length; j++) {
        if (google.maps.geometry.poly.containsLocation(new google.maps.LatLng(tab[i].lat, tab[i].lng), polygons[j])) {
            console.log("i=", i);
            nbMuseesParQuartier[indexes[j]-1]++;
        }
    }
...
console.log(nbMuseesParQuartier);
var s = "";
for (var i = 0; i < nbMuseesParQuartier.length; i++) {
    s += nbMuseesParQuartier[i]
    s += "\n"
}
console.log(s);
